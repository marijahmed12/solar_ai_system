...
def predict_next_hour():
    ...
    solar_decl = -23.44 * math.cos(
        math.radians(360 / 365 * (day + 10))
    )
    ...
    cosz = (
        math.sin(lat) * math.sin(math.radians(solar_decl))
    ...
    if cosz <= 0:
        ...
        dist = 1 + 0.033 * math.cos(
            math.radians(360 * day / 365)
        )
    ...
    def fitness(x):
        ...
        for i in range(n):
            ...
            if x[i] == 1:
                ...
                total += (
                    appliances[flexible[i]]['qty']
                    * appliances[flexible[i]]['watts']
                )
...
def optimize_next_hour(solar_watts, appliances):
    ...
    # ================= CRITICAL LOAD DISPLAY =================
    for name, data in appliances.items():

        if data['type'] == 'critical':

            power = data['qty'] * data['watts']

            result.append(
                f"🟢 {name.upper()} CRITICAL ON ({power}W)"
            )
    ...
    for name, state in best.items():
        ...
        power = (
            appliances[name]['qty']
            * appliances[name]['watts']
        )

        if state and available >= power:

            result.append(
                f"✅ {name.upper()} FULL ON ({power}W)"
            )
...
if __name__ == "__main__":
    ...
    result = optimize_next_hour(
        solar_watts,
        appliances
    )

    print("\n⚡ Critical Load:", result['critical'], "W")
    ...
    print("\n🔌 Grid Usage:", round(result['grid'], 2), "W")

    print(
    "🔋 Battery Save:",
    round(result['available'], 2),
    "W"
)